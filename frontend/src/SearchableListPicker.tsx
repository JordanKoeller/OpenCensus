import React, { useState, ReactNode, CSSProperties, ReactElement } from "react";
        ...
        >
          <FormControl
            ...
            onKeyPress={(e: React.KeyboardEvent<HTMLInputElement>) => {
              if (e.key === 'Enter') {
                // e.preventDefault();
                const chosenOption = props.options.findIndex((elem: string) => elem.toLowerCase().startsWith(value.searchTerm));
                setValue({
                  searchTerm: '',
                  selectionIndex: chosenOption
                });
                document.dispatchEvent(new MouseEvent('click'));
                return props.onSelected(props.options[chosenOption], e);
              }
            }}
